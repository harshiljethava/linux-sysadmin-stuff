...
echo '''>_ Please select options 
	...
	11) Check queue count of zabbix_server through zabbix_get 
	...
	21) Install Zabbix server
    22) Get zabbix_agent value from zabbix_get Command
    23) Check telnet to zabbix_agent
    24) Check ping to zabbix_agent
    25) Check snmp status to zabbix_agent'''
...
do
	...
	if [ $version == 'Y' ] || [ $version == 'y' ] || [ $version == 'yes' ] || [ $version == 'YES' ];then
        ...
        # ========================================================================== #
        elif [ $options -eq 21 ]; then
        	echo -e ">_ Installing Zabbix server ..."
        	echo -e ">_ Download  Zabbix installation file(s) from another server through scp"	
        	
	        echo -e ">_ Enter remote server ip"
	       	read inst_host
	       	echo -e ">_ Enter remote host user "
	       	read inst_user
	   		echo -e ">_ Enter name of directory contains zabbix installation file (absolute path)"
	   		read inst_path
	 		echo -e "\n>_ Copying Zabbix installation file(s) to home directory..."
       		scp_install="scp $inst_user@$inst_host:$inst_path ~"
       		eval $scp_install
	        echo -e "\n>_ Zabbix installation file(s) copied Successfully..."
			echo -e ">_ Installing Zabbix sever ..."
			echo -e ">_ Installing dependencies gcc curl net-snmp"
			depend="yum install gcc curl libcurl libcurl-devel net-snmp-devel openssl openssl-devel"
			eval $depend
			echo -e ">_ Required dependencies installed Successfully..."
			echo -e ">_ How you want to configure Zabbix server ?
						Please select below options
						a) with Encyption
						b) without Encyption"
			read encryp_opt
				if [ $encryp_opt -eq 'a' ] || [ $encryp_opt -eq 'A'];then
						zabbix_install="./configure --enable-server --enable-agent --with-mysql --enable-ipv6 --with-net-snmp --with-libcurl --with-libxml2 --with-openssl"
						eval $zabbix_install
						echo -e ">_ Zabbix server ( with Encyption support ) installed Successfully..."
                elif [ $encryp_opt -eq 'b'] || [ $encryp_opt -eq 'B']; then
                        zabbix_install="./configure --enable-server --enable-agent --with-mysql --enable-ipv6 --with-net-snmp "
                        eval $zabbix_install
                        echo -e ">_ Zabbix server ( without Encyption support ) installed Successfully..."
                fi
                else
                    echo -e "\nInvalid options.Run script again !!! "
                    exit 1
